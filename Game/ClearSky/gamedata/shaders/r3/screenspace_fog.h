...
#ifdef G_USE_PARAMS_FROM_WEATHER
	float fogheight = smoothstep(lowland_fog_params.x, -lowland_fog_params.x, WorldP.y) * G_FOG_HEIGHT_INTENSITY;
#else
	float fogheight = smoothstep(G_FOG_HEIGHT, -G_FOG_HEIGHT, WorldP.y) * G_FOG_HEIGHT_INTENSITY;
#endif
