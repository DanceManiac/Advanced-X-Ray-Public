...
float4 process_night_vision_2(p_screen I) : SV_Target
{
	...
	float warpLine = frac(+timers.x * (NVG_WARP_LINE_PERIOD_NVG_2 + ((device_influence.x + psy_zone_intensity + rad_zone_intensity) * 15)));
    ...
    */
	...
	float offset = sin(warpArg01 * (NVG_WARP_LINE_INTENSITY_NVG_2 + ((device_influence.x + psy_zone_intensity + rad_zone_intensity) * 15)))  * f1(warpArg01);
	
	
	float4 lineNoise = float4(1.0, 1.0, 1.0, 1.0);
	if(abs(uv.y - frac(+timers.x * (19.0 + ((device_influence.x + rad_zone_intensity) * 100)))) < 0.0005 + (device_influence.x / 100.0))
	{
	...
	// APPLY NOISE
	color += jitter.y * (0.1 + ((device_influence.x + psy_zone_intensity + rad_zone_intensity) / 2.0)); // Add the noise to the image
