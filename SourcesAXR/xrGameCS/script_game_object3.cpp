...
void CScriptGameObject::add_animation			(LPCSTR animation, bool hand_usage, bool use_movement_controller)
{
	...
	if (stalker->animation().global_selector()) {
		Msg("! Cannot add animation [%s]: global selector is set for object [%s], in_smart_cover returned [%s]",
			animation,
			stalker->cName().c_str(),
...
void CScriptGameObject::set_dest_level_vertex_id(u32 level_vertex_id)
{
	...
	if (!stalker->movement().restrictions().accessible(level_vertex_id)) {
		Msg("! you are trying to setup destination for the stalker %s, which is not accessible by its restrictors in[%s] out[%s]",
			stalker->cName().c_str(),
...
{
	RMakeObj(CAI_Stalker,stalker,DetailPathManager::eDetailPathTypeSmooth);
...
{
	RMakeObj(CCustomMonster,custom_monster,NULL);
