...
IC	bool CBlend::update_time			( float dt )
{
	...
	float	const END__EPS	=	SAMPLE_SPF+EPS;
	bool	at_end			=	running_fwrd && ( timeCurrent > ( timeTotal-END__EPS ) );
	...
	if( !stop_at_end )
	{
		...
		if( at_end )
			timeCurrent -= ( timeTotal-END__EPS );
	...
	if( at_end )
	{
		timeCurrent	= timeTotal-END__EPS;		// stop@end - time frozen at the end
