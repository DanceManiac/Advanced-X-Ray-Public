...
void CBlender_Hud_Acid_Rain::Compile(CBlender_Compile& C)
{
	...
	{
		case 0:
			C.r_Pass("null", "hud_fallout_acid_rain", FALSE, FALSE, FALSE);
			C.r_Sampler_clf("s_image", r2_RT_generic0);
			C.r_Sampler_clf("s_hud_acid_rain", "shaders\\hud_mask\\hud_fallout_acid_rain");
			C.r_End();
		break;
	}
}
...
void CBlender_Hud_Radiation_Rain::Compile(CBlender_Compile& C)
{
	...
	{
	case 0:
		C.r_Pass("null", "hud_fallout_radiation_rain", FALSE, FALSE, FALSE);
		C.r_Sampler_clf("s_image", r2_RT_generic0);
		C.r_Sampler_clf("s_hud_radiation_rain", "shaders\\hud_mask\\hud_fallout_radiation_rain");
		C.r_End();
		break;
	}
}