...
void CRenderTarget::phase_chrom_aberration_acid()
{
	u32 Offset = 0;
	float d_Z = EPS_S;
	float d_W = 1.0f;
	...
	// Draw COLOR
	RCache.set_Element(s_chromatic_aberration2->E[1]);
	RCache.set_c("chromatic_aberration_acid_params", envdesc.m_fWindVolumeFalloutAcid, droplets_power_debug, 0, 0);
	RCache.set_Geometry(g_combine);
	RCache.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);
