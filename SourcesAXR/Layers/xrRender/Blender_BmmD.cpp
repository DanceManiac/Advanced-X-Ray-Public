...
void	CBlender_BmmD::Compile	(CBlender_Compile& C)
{
	...
	bool z_prepass			= ps_r2_ls_flags_2.test(R2FLAG_TERRAIN_PREPASS);
	switch(C.iElement) 
	{
	case SE_R2_NORMAL_HQ: 		// deffer
		...
		uber_deffer		(C, true,	"impl", "impl", false, oT2_Name[0] ? oT2_Name : 0, z_prepass);
	...
	case SE_R2_NORMAL_LQ: 		// deffer
		...
		uber_deffer		(C, false,	"base", "impl", false, oT2_Name[0] ? oT2_Name : 0, z_prepass);
		if (z_prepass)	C.RS.SetRS(D3DRS_ZFUNC, D3DCMP_EQUAL);
		C.r_Sampler		("s_lmap",	C.L_textures[1]);
		C.r_End			();
		break;
