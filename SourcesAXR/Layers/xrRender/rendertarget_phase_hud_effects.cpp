...
void CRenderTarget::phase_hud_infections()
{
	//Constants
	u32 Offset = 0;
	u32 C = color_rgba(0, 0, 0, 255);

	float ActorCurrentInfection = g_pGamePersistent->GetActorInfection();

	//Because the values are inverted...
	ActorCurrentInfection = 1.0 - ActorCurrentInfection;

	if (ActorCurrentInfection < 0.8) //Red icon...
		return;
	...
	// Draw COLOR
	RCache.set_Element(s_hud_infections->E[0]);
	RCache.set_c("hud_infections_params", ActorCurrentInfection, 0, 0, 0);
	RCache.set_Geometry(g_combine);
	RCache.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);
