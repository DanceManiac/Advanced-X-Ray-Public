...
void Property::construct( shared_str const& property_id, Manager& manager_r )
{
	...
	VERIFY2( pSettings->section_exist( m_id ), make_string( "Section of upgrade property [%s] doesn't exist!", m_id.c_str() ) );
	...
	R_ASSERT2(
		...
		make_string( "Failed to get upgrade property functor in [%s], functor[%s]",
		id_str(), functor_str
		)
	);
	m_desc(); // test
