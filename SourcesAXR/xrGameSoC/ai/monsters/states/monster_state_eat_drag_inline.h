...
void CStateMonsterDragAbstract::initialize()
{
	...
	object->character_physics_support()->get_movement()->PHCaptureObject(const_cast<CEntityAlive *>(object->CorpseMan.get_corpse()));
	
	m_failed = false;
	
	CPHCapture *capture = object->character_physics_support()->get_movement()->PHCapture();
