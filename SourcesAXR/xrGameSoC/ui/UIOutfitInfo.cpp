...
void CUIOutfitItemInfo::SetValue(float value, float comp, int vle, int accuracy)
{
	...
	if (GameConstants::GetColorizeValues())
	{
		...
		{
#pragma todo("do not forget about disabled comparing here!")
			//if (value == comp)
			//{
			//	m_value->SetTextColor(m_neutral_color);
			//}
...
#pragma todo("do not forget about disabled comparing here!")
	...
	cur > 0 ? m_value->SetTextColor(positive_color) : m_value->SetTextColor(negative_color);
...
void CUIOutfitItem::SetInfo(CCustomOutfit* cur_outfit, CCustomOutfit* slot_outfit)
{
	...
	for (u32 i = 0; i < max_count; ++i)
	{
		...
		float value = cur_outfit->GetDefHitTypeProtection(hit_type);
		value = 1.0f - value;
		cur = value;
		if (fis_zero(cur))
			continue;
		...
		if (slot_outfit)
		{
			slot = 1.0f - slot_outfit->GetDefHitTypeProtection(hit_type);
		}
