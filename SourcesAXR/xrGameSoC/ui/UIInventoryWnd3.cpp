...
{
	if (lst == m_pUIOutfitList || lst == m_pUIHelmetList || lst == m_pUISecondHelmetList)
	{
...
void CUIInventoryWnd::PropertiesBoxForSlots(PIItem item, bool& b_show)
{
	...
	CCustomOutfit* outfit_in_slot = smart_cast<CCustomOutfit*>(m_pInv->ItemFromSlot(OUTFIT_SLOT));
	CHelmet* helmet_in_slot = smart_cast<CHelmet*>(m_pInv->ItemFromSlot(HELMET_SLOT));
	CHelmet* second_helmet_in_slot = smart_cast<CHelmet*>(m_pInv->ItemFromSlot(SECOND_HELMET_SLOT));
...
void CUIInventoryWnd::PropertiesBoxForUsing(PIItem item, bool& b_show)
{
	...
	CHelmet* item_in_helmet_slot = smart_cast<CHelmet*>(m_pInv->ItemFromSlot(HELMET_SLOT));
	CHelmet* item_in_helmet2_slot = smart_cast<CHelmet*>(m_pInv->ItemFromSlot(SECOND_HELMET_SLOT));
	CDetectorAnomaly* item_in_anomaly_detector_slot = smart_cast<CDetectorAnomaly*>(m_pInv->ItemFromSlot(DETECTOR_SLOT));
	...
	else if (pRepairKit)
	{
		...
		if (item_in_helmet_slot && can_repair_helmet)
		{
			...
			UIPropertiesBox->AddItem(str.c_str(), (void*)item_in_helmet_slot, REPAIR_KIT_HELMET, repair_hint.c_str());
			b_show = true;
		}
		...
		if (item_in_helmet2_slot && can_repair_helmet2)
		{
			...
			UIPropertiesBox->AddItem(str.c_str(), (void*)item_in_helmet2_slot, REPAIR_KIT_SECOND_HELMET, repair_hint.c_str());
			b_show = true;
		}
	...
	else if (pFilter)
	{
		...
		if (item_in_helmet_slot && helmet_use_filter)
		{
			...
			str.printf("%s %s", str.c_str(), item_in_helmet_slot->m_name.c_str());
			UIPropertiesBox->AddItem(str.c_str(), (void*)item_in_helmet_slot, FILTER_CHANGE_HELMET);
			b_show = true;
		}
		...
		if (item_in_helmet2_slot && helmet2_use_filter)
		{
			shared_str str = CStringTable().translate("st_change_filter");
			str.printf("%s %s", str.c_str(), item_in_helmet2_slot->m_name.c_str());
			UIPropertiesBox->AddItem(str.c_str(), (void*)item_in_helmet2_slot, FILTER_CHANGE_HELMET);
			b_show = true;
		}
